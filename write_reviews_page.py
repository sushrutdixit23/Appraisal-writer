...
//
// Lifecycle is forward-only by design, matching the roadmap's diagram
// (no backward arrows shown): Waiting for Data -> Data Imported ->
// Analysis Complete -> Ready for Review -> Management Review ->
// Completed -> Archived. Reaching Completed stamps closed_at; Archived
// is a manual final step, not automatic, so a completed cycle stays
// visible/editable until someone deliberately archives it.
...
// labels. A cycle's status must be one of these values; "advance"
// always means "move to the next entry in this array".
const LIFECYCLE: { value: ReviewCycleStatus; label: string }[] = [
  { value: "waiting_for_data", label: "Waiting for Data" },
...
export default function ReviewCyclesPage() {
  ...
  async function createCycle() {
    ...
    const { data, error: insertError } = await supabase
      ...
      .insert({
        ...
        status: "waiting_for_data" as ReviewCycleStatus,
  ...
  async function advanceCycle(cycle: ReviewCycle) {
    ...
    // Reaching Completed stamps closed_at; if a cycle somehow jumps
    // straight to Archived without ever passing through Completed
    // (shouldn't happen via this UI, but don't leave closed_at unset
    // if it does), stamp it there too.
    if ((next === "completed" || next === "archived") && !cycle.closed_at) {
