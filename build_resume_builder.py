route_content = """import Anthropic from "@anthropic-ai/sdk";
...
import os
os.makedirs("app/api/resume-build", exist_ok=True)
with open("app/api/resume-build/route.ts", "w", encoding="utf-8", newline="\n") as f:
    f.write(route_content)
print("Created app/api/resume-build/route.ts")
...
os.makedirs("app/resume-builder", exist_ok=True)
with open("app/resume-builder/page.tsx", "w", encoding="utf-8", newline="\n") as f:
    f.write(page_content)
print("Created app/resume-builder/page.tsx")

def replace_once(text, old, new, label):
    count = text.count(old)
    if count != 1:
        raise SystemExit(f"FAILED anchor '{label}': found {count} matches, expected 1")
    return text.replace(old, new, 1)

with open("app/ats-checker/page.tsx", "r", encoding="utf-8") as f:
    ats = f.read()

old_footer = """              <button onClick={reset} className=\"text-[13px] text-slate hover:text-indigo transition-colors\">&larr; Check another resume</button>
              <a href=\"/appraisal-writer\" className=\"text-[13px] text-indigo hover:underline\">Also try the Appraisal Writer &rarr;</a>"""
new_footer = """              <button onClick={reset} className=\"text-[13px] text-slate hover:text-indigo transition-colors\">&larr; Check another resume</button>
              <a href=\"/resume-builder\" className=\"text-[13px] text-indigo hover:underline\">Build an ATS-optimized version &rarr;</a>"""
ats = replace_once(ats, old_footer, new_footer, "ats-checker footer link")

with open("app/ats-checker/page.tsx", "w", encoding="utf-8", newline="\n") as f:
    f.write(ats)
print("Linked ats-checker results to /resume-builder")
