...
function CreditsChip({ credits }: { credits: number }) {
  return (
    <div className="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-indigo/25 bg-indigo/8" style={{ background: "rgba(91,75,255,0.07)" }}>
      <svg width="14" height="14" viewBox="0 0 100 100" fill="none" style={{ filter: "drop-shadow(0 0 4px rgba(91,75,255,0.9)) drop-shadow(0 0 8px rgba(74,158,255,0.6))", flexShrink: 0 }}>
        <defs>
          <linearGradient id="navg" x1="20" y1="0" x2="80" y2="100" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor="#4A9EFF"/>
            <stop offset="45%" stopColor="#5B4BFF"/>
            <stop offset="100%" stopColor="#7B2FE0"/>
          </linearGradient>
        </defs>
        <path d={STAR_PATH} fill="url(#navg)"/>
          ...
          <div className="flex items-center gap-3">
            ...
            {session && (
              <button
                ...
                className={`hidden md:inline-flex px-4 py-2.5 rounded-[11px] text-[14px] text-ink-soft border border-line hover:text-indigo hover:border-indigo ${btn}`}
              >
                Sign out
              </button>
            )}
          ...
          <div className="relative w-full max-w-sm bg-cloud border border-line rounded-[24px] p-7 shadow-zy-lg" onClick={(e) => e.stopPropagation()}>
            ...
            {status === "sent" ? (
              ...
              <div className="space-y-3">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleEmailAuth()} placeholder="your@email.com"
                  className="w-full bg-mist border border-line rounded-xl px-4 py-3 text-ink text-sm placeholder:text-slate focus:outline-none focus:ring-2 focus:ring-indigo" />
                {errorMsg && <p className="text-rose text-[12px]">{errorMsg}</p>}
                <button onClick={handleEmailAuth} disabled={status === "sending"}
