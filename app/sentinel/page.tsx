...
// Investigation Engine V2 - named_peer/possible_causes/financial_impact
// are all optional (null/[] on investigations generated before this was
// added), so each section only renders when present. Shared between the
// pending-review view and the approved/edited view - useful context in
// both, not just during review.
function InvestigationExtras({ inv }: { inv: Investigation }) {
  return (
    <>
      {inv.named_peer && (
        <p style={{ fontSize: "0.8rem", color: T.inkSoft, margin: "0.4rem 0 0 0" }}>
          Closest comparable peer: <strong style={{ color: T.ink }}>{inv.named_peer}</strong>
        </p>
      )}
      {inv.financial_impact && (
        <>
          <Eyebrow>Financial impact</Eyebrow>
          <p style={{ fontSize: "0.93rem", lineHeight: 1.65, margin: 0 }}>{inv.financial_impact}</p>
        </>
      )}
  ...
  return (
    <div>
      ...
      {visibleGroups.map((group) => {
        ...
        const isHighlighted = workspaceId === highlightWorkspaceId;
